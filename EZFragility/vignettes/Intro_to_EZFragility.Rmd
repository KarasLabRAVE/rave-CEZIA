...
lambda <- NULL
nSearch <- 10
pt01Fragm1sp2s <- calcAdjFrag(ieegts = pt01Epochm1sp2s, tWindow = tWindow, tStep = tStep, lambda = lambda,nSearch=nSearch)
...
timeWindow <- c(-1,2)
display <- c(sozIndex,77:80)
fragplot<-heatmapFrag(frag=pt01Fragm1sp2s,elecSoz=sozIndex,timeWindow = timeWindow,title="PT01 seizure 1",display=display)
fragplot
```
...
fragstat <- fragStat(frag=pt01Fragm1sp2s, elecSoz=sozIndex)
fragstat
```
...
pfragstat<-plotFragDistribution(stat =fragstat,timeWindow=timeWindow)
pfragstat
```
...
qfragplot<-plotFragQuantile(FragStatObj=fragstat, timeWindow=timeWindow)
qfragplot
```